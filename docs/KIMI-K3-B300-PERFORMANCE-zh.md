...
> TTFT p50 偏低 5–19%。
>
> **各档共用同一深度并不等于扫描自洽。** 一次运行达到稳态所*需*的深度随并发上升而
> 下降（他处实测：c20 需 23.3 req/slot，c40 只需 14.7），所以把同一个
> `12 × 并发数` 用在 1/8/16/32/64 上，相对各档的需求并不等深 —— 低并发端很可能偏浅、
> 高并发端才够。凡是**跨并发点**比较的结论（效率拐点、吞吐曲线形状）都在上述
> ramp-up 污染之外额外继承了这一不均匀性，应视为暂定。各档数字按实测保留。
