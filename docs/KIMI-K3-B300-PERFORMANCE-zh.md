...
`log_stats` 的样本中位值 —— 取样区间为 `Reqs Running` 达到目标并发期间（每档 18–64 个
样本）。⚠️ 这是**加载期内的引擎侧中位值，不是稳态值**：`Reqs Running` 达到目标只说明
队列已满，那是进入稳态的前提而非证据 —— 吞吐仍可能在漂移。这些样本未做任何时间稳定性
检验。
...
**上述退化期间 KV cache 使用率峰值为 20.8%，preemption 计数为 0。** 加载期
